...
def check_other_server_health():
    """Heartbeat capa 7 porque el dolor pudo más que el oro
...
def monitor_other_server():
    """
    Heartbeat en otro hilo, como cada minuto porque t2.micro con varios hilos en python es mucho pedir
    """
    while True:
        if not check_other_server_health():
            ...
            update_thread = threading.Thread(
                target=periodic_file_list_update, args=(update_interval,)
            )
...
def get_server():
    """Retorna el siguiente servidor al cual se va a usar con estrategia de round robin.

    Yields:
        str: IP de un dataNode
    """
...
def get_next_server(private=False):
    ...
    Hace lo mismo que get_server() pero peor
    Un cycle entre server addresses con yield hace la misma vaina pero
    un merge conflict se llevó eso por delante.
...
def periodic_file_list_update(interval):
    """Recrea el file list desde 0 cada vez, periodicamente
...
def upload_file(filename):
    try:
        ...
        response = requests.post(
            f"{server_url}", data=request.data, headers=request.headers
        )
...
def get_server_for_file(filename):
    """Busca un servidor donde esté el archivo especificado

    Args:
        filename (str)

    Returns:
        str | None: devuelve la ip del servidor donde está, si es que está.
    """
    for server_address in grpc_addresses:
...
def list_files():
    try:
        # Aggregate the file lists from all dataNode servers into one unified list
        all_files = []
